...
export const PlanCard = ({ plan, onSelect }: PlanCardProps) => {
  return (
    <Card 
      className={`
        rounded-2xl border border-border/70 transition-all duration-300 relative
        ${plan.popular 
          ? 'bg-card shadow-lg scale-105 md:scale-110 z-10' 
          : 'bg-card/50 hover:shadow-md'}
      `}
    >
      {plan.popular && (
        <div className="absolute -top-2.5 left-1/2 transform -translate-x-1/2 px-4 py-1 bg-primary text-primary-foreground text-xs font-medium rounded-full whitespace-nowrap z-20">
          Most Popular
        </div>
      )}
